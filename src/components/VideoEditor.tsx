...
const VideoEditor: React.FC = () => {
  ...
  // Handle right sidebar enhancement actions
  const handleGreenScreen = () => {
    setGreenScreenEnabled(prev => !prev);
    toast.success(greenScreenEnabled ? 
      "Green screen disabled" : 
      "Green screen enabled! Upload a video with a solid background color."
    );
  };

  const handleMagicResize = () => {
    setAspectRatio(prev => prev === 'landscape' ? 'portrait' : 'landscape');
    toast.success(`Video resized to ${aspectRatio === 'landscape' ? 'portrait' : 'landscape'} format`);
  };

  const handleAutoCaptions = () => {
    setAutoCaptionsEnabled(true);
    toast.success("Generating auto captions...");
    
    // Simulate caption generation
    setTimeout(() => {
      toast.success("Auto captions generated successfully!");
    }, 2000);
  };
          ...
          {(editorState) => (
            <div className="flex flex-1 p-6 bg-gradient-to-br from-purple-100 via-white to-fuchsia-100">
              ...
              <EditorRightSidebar
                ...
                handleAutoCaption={handleAutoCaptions}
                handleGreenScreen={handleGreenScreen}
                handleMagicResize={handleMagicResize}
              />
            </div>
          )}
        </EditorStateProvider>
